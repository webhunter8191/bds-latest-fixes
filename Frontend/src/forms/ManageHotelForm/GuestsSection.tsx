...
// Define the RoomAvailability type
...
// Define the existingRooms type with optional availability
type existingRooms = {
  category: string;
  totalRooms: number;
  availableRooms: number;
  ...
  adultCount: number;
  childCount: number;
  availability?: RoomAvailability[]; // availability can be undefined
};
...
const GuestsSection = ({
  ...
  // Room categories and their titles
  const categories = [
    { value: "1", label: "2 Bed AC" },
    { value: "2", label: "2 Bed Non-AC" },
    { value: "3", label: "3 Bed AC" },
  ...
  useEffect(() => {
    if (existingRooms) {
      ...
      const updatedRooms = existingRooms.map((room) => ({
        ...room,
        availability: room.availability || [], // default to empty array if undefined
      }));
      setRooms(updatedRooms);
    }
  }, [existingRooms]);
  ...
  const handleSaveRoom = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const newRoom = {
      ...
      adultCount: Number(formData.get("adultCount")),
      childCount: Number(formData.get("childCount")),
      availability: rooms[editingRoomIndex || 0]?.availability || [], // preserve if any
    };

    if (editingRoomIndex !== null) {
      let updatedRooms = [...rooms];
      updatedRooms[editingRoomIndex] = { ...newRoom, availability: newRoom.availability || [] }; // Ensure availability is always an array
      setValue(
        "rooms",
        updatedRooms.map((room) => ({
          ...room,
          availability: room.availability || [], // Ensure availability is always an array
        }))
      );
      setRooms(updatedRooms);
      setEditingRoomIndex(null);
...
function setIsDialogOpen(arg0: boolean) {
  ...
  return (
    <div className="mb-8">
        ...
        {rooms.map((room, index) => (
          <div key={index} className="p-4 border rounded-lg mb-4">
            ...
            {room.availability && room.availability.length > 0 && (
              <div className="mt-4">
                <h4 className="font-semibold mb-2">Date-wise Availability</h4>
                <div className="space-y-1 text-sm text-gray-700">
                  {room.availability.map(
                    (entry: RoomAvailability, i: number) => (
                      <div key={i}>
                        <span className="font-medium">{entry.date}:</span> ₹
                        {entry.price} | {entry.availableRooms} rooms
                      </div>
                    )
                  )}
                </div>
      ...
      {/* Dialog for Add/Edit Rooms */}
      {isDialogOpen && (
        <Dialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)}>
          {/* Dialog content here */}
          <form onSubmit={handleSaveRoom}>
            {/* Form content for room details */}
...
export default GuestsSection;
function setRooms(updatedRooms: any) {
  throw new Error("Function not implemented.");
}

function setEditingRoomIndex(arg0: null) {
  throw new Error("Function not implemented.");
}
